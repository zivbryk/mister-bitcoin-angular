...
export class ContactService {
  ...
  public contacts$: Observable<Contact[]> = this._contacts$.asObservable()
  ...
  public query() {
    ...
    const contatcs = this._contactsDb
      .filter(({ name }) => {
        return name.toLowerCase().includes(filterBy.term.toLowerCase());
      });
    const sortedContacts = this._sort(contatcs)
    this._contacts$.next(sortedContacts);
  }

  // public loadContacts(filterBy: null | {} = null): void {
  ...
  // public getImgUrl(contactName) {
  //   return this.http.get<string>(`https://robohash.org/${contactName.replace(' ', '')}?set=set5`)
  //     .pipe()
